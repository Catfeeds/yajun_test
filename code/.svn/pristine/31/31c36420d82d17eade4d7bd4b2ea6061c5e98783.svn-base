...
@Controller
@RequestMapping(value = "/parameterManageDetail")
public class TmParameterManageDetailController extends BaseController {

	@Autowired
	private TmParameterManageDetailService service;
	@Autowired
	...
	public JsonActionResult add(HttpServletResponse response, TmParameterManageDetail bean) {
		bean = service.doSave(bean);
		logService.doAddLog("TmParameterManageDetail", bean);
	...
	public JsonActionResult update(HttpServletResponse response, TmParameterManageDetail bean) {
		TmParameterManageDetail before = service.findById(bean.getId());
		service.doUpdate(bean);
		logService.doUpdateLog("TmParameterManageDetail", before, bean);
		return ActionUtils.handleResult(response);
	}
	...
	public JsonActionResult delete(HttpServletResponse response, String ids) {
		...
		List<TmParameterManageDetail> list = service.findAllInIds(deleteIds);
		service.doDeleteById(deleteIds);
		logService.doDeleteLog("TmParameterManageDetail", list);
		return ActionUtils.handleResult(response);
	}
}
