...
@RequestMapping(value = "/repair/uloc/part")
public class TpRepairUlocPartController extends BaseController {

	@Autowired
	private TpRepairUlocPartService service;
	@Autowired
	...
	public JsonActionResult add(HttpServletResponse response, TpRepairUlocPart bean) {
		bean = service.doSave(bean);
		logService.doAddLog("TpRepairUlocPart", bean);
	...
	public JsonActionResult update(HttpServletResponse response, TpRepairUlocPart bean) {
		TpRepairUlocPart before = service.findById(bean.getId());
		service.doUpdate(bean);
		logService.doUpdateLog("TpRepairUlocPart", before, bean);
		return ActionUtils.handleResult(response);
	}
	...
	public JsonActionResult delete(HttpServletResponse response, String ids) {
		...
		List<TpRepairUlocPart> list = service.findAllInIds(deleteIds);
		service.doDeleteById(deleteIds);
		logService.doDeleteLog("TpRepairUlocPart", list);
		return ActionUtils.handleResult(response);
	}
}
