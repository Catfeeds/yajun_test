...
@Component(value = "station102ReadService")
public class Station102ReadService implements ReadCallBack {
	...
	public synchronized void doChanged(List<Item> itemList, String groupCode) throws JIException {
		try {
			...
			if (flag) {
				...
				OpcHelper.sendDataToApp("plcRfidRead", groupCode, outOrIn);//通过webservice调用接口
			}
