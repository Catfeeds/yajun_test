(function(b) {
    ...
    a.props = function(h, f) {
        ...
        var d = function(n, m, o) {
                ...
                //add liuzejun
                if(o.getId().indexOf("rect")!=-1){
                	var buttonPanel = '<button class="btn btn-info" id="bindPartButton">关键件信息</button>&nbsp;'
        									+'<button class="btn btn-info" id="bindParameterButton">参数信息</button>&nbsp;'
        									+'<button class="btn btn-info" id="bindSipButton">质检项信息</button>';
                	$("#buttonPanal").html(buttonPanel);
                	$("#buttonPanal").show();
                    b("#bindPartButton").bind("click",function(){
                    	var url = $("#baseUrl").val()+"/path/uloc/part/graph_listInput.do?currentPageId=pathUlocPartGraphGridId&rectSeq="
                    					+o.getId()+"&tmPathUlocId="+m.tmPathUlocId.value+"&tmPlantId="+$("#plantSelect").val();
                    	createdetailwindow("关键件信息", url, "pathUlocPartGraphGridId");
                    });
                    b("#bindParameterButton").bind("click",function(){
                    	var url = $("#baseUrl").val()+"/path/uloc/parameter/graph_listInput.do?currentPageId=pathUlocParameterGraphGridId&rectSeq="
    									+o.getId()+"&tmPathUlocId="+m.tmPathUlocId.value+"&tmUlocId="+m.ulocId.value;
                    	createdetailwindow("参数信息", url, "pathUlocParameterGraphGridId");
                	});
                    b("#bindSipButton").bind("click",function(){
                    	var url = $("#baseUrl").val()+"/path/uloc/sip/graph_listInput.do?currentPageId=pathUlocSipGraphGridId&rectSeq="
    									+o.getId()+"&tmPathUlocId="+m.tmPathUlocId.value+"&tmUlocId="+m.ulocId.value;
                    	createdetailwindow("质检项信息", url, "pathUlocSipGraphGridId");
                	});
                }else{
                	$("#buttonPanal").hide();
                }
                ...
                c.show();
                for (var l in m) {
                    if((m[l].readOnly)){//update by steven 
                    	...
                    	if(m[l].label=="NULL"){
                    		e.append("<tr style='display:none'><th>" + m[l].label + '</th><td><div id="p' + l + '" class="editor"></div></td></tr>');
                    	}else{
                    		e.append("<tr><th>" + m[l].label + '</th><td><div id="p' + l + '" class="editor"></div></td></tr>');
                    	}
                        if (m[l].editor) {
                ...
                //update by steven  
                if (!!a.config.editable&&(!m.tmPathId||o.getId().indexOf("rect")!=-1)) {//update by liuzejun
                    e.append("<tr><td>&nbsp;</th><td>"
                    		+'<div class="editor" align="right"><input type="button" id="_deleteNodeBtn" value="删除" /> </div></td></tr>');
                    
                  //update by liuzejun
                    b("#_deleteNodeBtn").bind("click",function(){
                    	if(m.tmPathUlocId&&m.tmPathUlocId.value){
                    		deleteCurrentNode(m.tmPathUlocId.value);
                    	}else{
                    		deleteCurrentNode("null");
                    	}
                    });
                } 
            };
         //update by steven   
            //update by liuzejun
        var deleteCurrentNode=function(delPathUlocId){
            var s="#p"+a.config.nodeIdName+" input", nodeId=(a.config.nodeIdName && typeof a.config.nodeIdName ==="string")?(b(s).val()):null;
            if(a.config.onDeleteNode && a.config.onDeleteNode(nodeId)){
            	var e=b.Event("keydown");
            	e.keyCode=460;
            	b(document).trigger(e);
            }
            if(delPathUlocId!="null"){
            	 deleateTmPathUlocIds.push(delPathUlocId);
            }
        }
    ...
    a.init = function(x, r) {
        ...
        if (a.config.editable) {
            ...
            b(x).droppable({
                ...
                drop: function(c, i) {
                	//从tools中拖拽出来.
                	console.log("droppable.left:"+i.helper.offset().left);
                	console.log("droppable.top:"+i.helper.offset().top);
                    b(y).trigger("addrect", [i.helper.attr("type"),
                    {
                        attr: {
                            x: i.helper.offset().left-40,
                            y: i.helper.offset().top-50
//                            x: i.helper.offset().left,
//                            y: i.helper.offset().top
                        },
                        text:{
                        	text:i.helper.context.dataset.value
                        },
                        props:{
                        	ulocNo:{
                            	value:i.helper.context.dataset.value
                            },
                            ulocId:{
                            	value:i.helper.context.dataset.id
                            },
                            isSip:{value:i.helper.context.dataset.issip}
                        }
                    }])
                }
            });
        ...
        if (r.restore) {
            ...
            if(B.props){
            	 a.config.props.props = b.extend(true, {}, a.config.props.props,B.props.props);
            }
        }
